...
makeGlobalData <- function(APP_DATA) {
  ...
  featPlotData <- lapply(list("mm10" = "mm10", "hg38" = "hg38"), function(gen) {
    ...
    return(
      list(
        ...
        "prediction" = plotEnrichment(rlr,
          splitby = "prediction", label_POS_only = FALSE,
          returnData = TRUE, pred_POS_only = FALSE,
          rlbaseRes = rlbaseRes, rlsamples = rlsamples
        ),
        "label" = plotEnrichment(rlr,
          splitby = "label",
  ...
  hg38sampsfls <- setNames(lapply(hg38samps, function(x) {
    file.path("misc", "rlranges", paste0(x, "_hg38.rds"))
  }), nm = hg38samps)
...
runrlseq <- function(inputs, awstry_put, awstry_saverds) {
  ...
  timestamp()
  a_ <- lapply(names(inputs), function(x) {
    message(x, ": ", inputs[[x]])
  })
...
awstry_put <- function(file, object, bucket, acl) {
  ...
  while (attempt > 0 & !success) {
    ...
    res <- try(aws.s3::put_object(
      file = file, object = object, verbose = TRUE,
      show_progress = TRUE, bucket = bucket, acl = acl
    ), silent = TRUE)
...
awstry_saverds <- function(x, compress, object, bucket, acl) {
  ...
  while (attempt > 0 & !success) {
    message("Attempt: ", attempt)
    res <- try(aws.s3::s3saveRDS(
      x = x, compress = compress, object = object, verbose = TRUE,
      show_progress = TRUE, bucket = bucket, acl = acl
    ), silent = TRUE)
    if ("try-error" %in% class(res)) {
      attempt <- attempt - 1
...
rlseqbg <- function(inputs, runrlseq, awstry_put = awstry_put,
  ...
  while (failed & attempts > 0) {
    ...
    timeoutfail <- c(
      "Error in get_result(output = out, options) : callr timed out\n",
  ...
  if ("try-error" %in% class(fail)) {
    ...
    aws.s3::put_object(
      file = inputs$tmpHTML1, object = file.path(inputs$runID, "res_index.html"),
      bucket = inputs$USERDATA_S3, acl = "public-read"
    )
  }
}
