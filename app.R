...
rlsamples <- rlsamples %>%
  mutate(
    rlsampleLink = map_chr(rlsample, makeSRAExperimentLinks),
...
rltabShow <- rlregions %>%
  ...
  mutate(
    avgSignalVal = signif(avgSignalVal, 4),
    avgQVal = 10^(-1 * avgQVal),
    confidence_score = signif(confidence_score, 4)
  ) %>%
  dplyr::select(
    `RL Region` = rlregion, Location = location,
  ...
  mutate(
    allGenes = gsub(allGenes, pattern = ",", replacement = " ", perl = TRUE),
...
ui <- function(request) {
  tagList(
    ...
    navbarPage(
      ...
      tabPanel(
        title = "Home", id = "home-tab", value = "aboutTab", icon = icon("home"),
        fluidPage(
          br(),
      ...
      tabPanel(
        title = "Samples", id = "samples-tab", icon = icon("vials"),
        SamplesPageContents(rlsamples)
      ),
      tabPanel(
        title = "R-Loop Regions", id = "rloops-tab", icon = icon("map"),
        RLoopsPageContents()
      ),
      tabPanel(
        title = "Analyze", id = "analyze-tab", icon = icon("server"),
        AnalyzePageContents(rlsamples)
      ),
      tabPanel(
        title = "Download", id = "download-tab", icon = icon("download"),
        DownloadPageContents(bucket_sizes, rlsamples)
      ),
      tabPanel(
        title = "Documentation", id = "docs-tab", icon = icon("file-alt"),
        tags$iframe(
...
server <- function(input, output, session) {
  ...
  rmapSampsRV <- reactive({
    rlsamples %>%
      dplyr::filter(
        .data$genome == input$selectGenome,
  ...
  output$rmapSamples <- renderDT(server = FALSE, {
    ...
    rlsamples %>%
      dplyr::filter(rlsample %in% rmapSampsRV()) %>%
      dplyr::select(
        Sample = rlsampleLink, Study = study, Mode = mode, Tissue = tissue, Condition = condition,
  ...
  output$sampleSummary <- renderUI({
    ...
    currentrlsample <- mutate(currentrlsample, across(contains("_S3"), function(x) {
      paste0(
        "<a href='", file.path(RLSeq:::RLBASE_URL, x), "' target='_blank' >",
      ...
      column(
        ...
        tribble(
          ...
          "Label", paste0(
            ...
            currentrlsample$label, "</strong>"
          ), "The label 'POS' or 'NEG' referencing whether the sample was expected to map R-loops.",
          "Prediction", paste0(
            "<strong style='color: ", ifelse(currentrlsample$prediction == "POS", "#165566", "#8a2c2c"), "'>",
            currentrlsample$prediction, "</strong>"
          ), "The prediction ('POS' or 'NEG') made by the quality model (see details).",
  ...
  donuts <- reactive({
    ...
    modeDat <- dplyr::mutate(rlsamplesNow, Mode = ifelse(mode %in% RLSeq:::auxdata$mode_cols$mode, mode, "misc")) %>%
      group_by(Mode) %>%
      tally()
    labelDat <- dplyr::rename(rlsamplesNow, Label = label) %>%
      group_by(Label) %>%
      tally()
    ...
    pltLst <- lapply(names(datList), function(dat) {
      ...
      plot_ly(type = "pie") %>%
        add_pie(
          data = dataNow, labels = dataNow[, dat, drop = T], values = ~n, textinfo = "label+value",
  ...
  output$heatmap <- renderPlot({
    ...
    pheatmap(corrRes,
      color = heatData$pheatmap_color, main = current_samp(), breaks = heatData$pheatmap_breaks,
  ...
  output$rmapPCA <- renderPlot({
    ...
    ggplot(
      toPlt,
      aes_string(
        x = "PC1", y = "PC2", color = "Mode",
        alpha = "group",
        shape = input$PCA_shapeBy, size = "group"
      )
    ) +
      rlbase_scatter(
        sizes = c("Selected" = 10, "Unselected" = 2.5),
  ...
  output$RLoopsPerSample <- renderDT({
    ...
    rltabNow %>%
      ...
      dplyr::relocate(Genes, .after = `Confidence Score`) %>%
      DT::datatable(
        extensions = "Buttons", selection = list(mode = "none"), rownames = FALSE,
  ...
  output$RLoopsSummary <- renderUI({
    rloopsNow <- dplyr::filter(rloops(), `RL Region` == current_rl()) %>%
      mutate(Genes = map_chr(Genes, function(x) {
        paste0(sapply(unique(unlist(strsplit(Genes, split = " "))), makeGeneCards), collapse = "\n")
      })) %>%
      mutate(Genes = ifelse(Genes == NA_LINK, NA, Genes)) %>%
      mutate(Samples = map_chr(Genes, function(x) {
        paste0(sapply(unique(unlist(strsplit(samples, split = ","))), makeSRAExperimentLinks), collapse = "\n")
      })) %>%
    ...
    rloopsNow %>%
      ...
      mutate(message = c(
        ...
        paste0(
          "The source type. RL Regions were calculated separately for S9.6 and dRNH-based modalities.",
  ...
  output$RLvsExpbySample <- renderPlot({
    ...
    dplyr::filter(tpm_rl_exp, rlregion == current_rl()) %>%
      ...
      dplyr::rename(
        Other = other, Mode = mode, Tissue = tissue,
  ...
  observeEvent(input$userUpload, {
    ...
    inputs <- c(fls, setNames(list(
      hash, input$userSample, input$userGenome,
    ...
    aws.s3::put_object(
      file = tmpHTML1, object = file.path(hash, "res_index.html"),
      bucket = USERDATA_S3, acl = "public-read"
    )
    shinyWidgets::sendSweetAlert(
      session = session, title = "Running.", html = TRUE, type = "info",
      text = tags$span(
        "You results will be available here when ready:",
    ...
    rprocess <- callr::r_bg(
      ...
      args = list(
        inputs = inputs, runrlseq = runrlseq,
    ...
    updateActionButton(session,
      inputId = "userUpload", label = "Running...",
    ...
    if (dt[[1]] < -300 & rp$format() != "PROCESS 'R', finished.\n") {
      ...
      shinyWidgets::sendSweetAlert(
        ...
        text = tags$span(
          ...
          tags$strong(
            "If you have recieved this error previously, please submit an issue to the ",
      ...
      updateActionButton(session,
        inputId = "userUpload", label = "Start",
  ...
  output$rlsamplesDownloadFiles <- renderDT({
    rlsamples %>%
      ...
      mutate(across(contains("("), function(x) {
        paste0(
          "<a href='", file.path(RLSeq:::RLBASE_URL, x),
          "' target='_blank' download><i class='fa fa-download'></i>  ",
          toupper(gsub(basename(x),
            pattern = "^.+\\.([a-zA-Z0-9]+)$",
